          ...
          <div class="inner-header">
            <b-row>
              ...
              <b-col cols="1" class="text-left">
                <router-link :to="{name: 'client-users'}" class="btn btn-danger w-100">بازگشت</router-link>
              </b-col>
            </b-row>
          </div>
...
export default {
  data() {
    return {
      ...
      file_location: "/avatars/chival/users",
  ...
  mounted() {
    this.getWorkTime();
    this.getRoles();
    this.file_location = "/company/"+this.isUserData['company_id']+"/avatars";
    if (this.$route.params.id) {
      this.isEdit = true;
      let promise = this.$http.get(
        this.$api("api/v1/client/users/" + this.$route.params.id)
      );
      promise
        .then(response => {
          this.form = response.data;
          this.role_id = {id: response.data.role_id, name: response.data.role};
        })
        .catch(error => {
  ...
  methods: {
    ...
    getRoles() {
      let promise = this.$http.get(this.$api("api/v1/client/roles"));
      return promise
        .then(response => {
    ...
    getWorkTime() {
      let promise = this.$http.get(this.$api("api/v1/client/worktimes/list"));
      return promise
        .then(response => {
      ...
      if (this.$route.params.id) {
        ...
        promise
          .then(response => {
            ...
            this.$router.push(this.$route.query.redirect || {name: 'client-users'});
          })
          .catch(error => {
        ...
        let promise = this.$http.post(
          this.$api("api/v1/client/users/new"),
          this.form
        );
        promise
          .then(response => {
