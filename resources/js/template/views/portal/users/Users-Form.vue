          ...
          <div class="inner-header">
            <b-row>
              ...
              <b-col cols="1" class="text-left">
                <router-link to="/portal/users" class="btn btn-danger w-100">بازگشت</router-link>
              </b-col>
            </b-row>
          </div>
...
export default {
  ...
  mounted() {
    ...
    if (this.$route.params.id) {
      ...
      let promise = this.$http.get(
        this.$api("api/v1/portal/users/" + this.$route.params.id)
      );
      promise
        .then(response => {
  ...
  methods: {
    ...
    getRoles() {
      let promise = this.$http.get(this.$api("api/v1/portal/roles/chival"));
      return promise
        .then(response => {
    ...
    getWorkTime() {
      let promise = this.$http.get(this.$api("api/v1/portal/worktimes/chival"));
      return promise
        .then(response => {
    ...
    onSubmit(evt) {
      ...
      if (this.$route.params.id) {
        ...
        let promise = this.$http.post(
          this.$api("api/v1/portal/users/new"),
          this.form
        );
        promise
          .then(response => {
            console.error(response);
            this.$router.push(this.$route.query.redirect || "/portal/users/");
          })
          .catch(error => {
            this.isAlert = this.dismissSecs;
            let e = error.response;
