...
<section class="page">
  <div class="row">
    <CardProgress class="col-4 offset-8" url="/portal/banks/new" type="default" :value="100" :striped="false" :animated="true">
      <template slot="header">
        <h5 class="mb-3">افزودن بانک و حساب بانکی جدید</h5>
      </template>
    </CardProgress><!-- -->
    ...
    <div class="col-12">
      ...
      >
        ...
        <template v-slot:cell(actions)="data">
          <router-link :to="{name: 'portal-banks-edit', params: { id: data.item.id }}"><i class="fas fa-pencil-alt"></i></router-link>
...
export default {
  ...
  data() {
    return {
      fields: [
        { key: 'name', label: 'بانک', sortable: true },
        { key: 'account', label: 'شماره حساب', sortable: false },
        { key: 'iban', label: 'شبا', sortable: false },
        { key: 'card', label: 'کارت', sortable: false },
        { key: 'owner', label: 'صاحب حساب', sortable: false },
  ...
  methods: {
    ...
    deleteItemOK(){
      let promise = this.$http.delete(this.$api("api/v1/core/banks/"+this.modalDelete.itemID))
    ...
    myProvider (ctx) {
      ...
      let promise = this.$http.get(this.$api("api/v1/core/banks"+params))
