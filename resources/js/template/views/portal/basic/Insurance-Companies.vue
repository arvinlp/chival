...
<section class="page">
  <div class="row">
    <CardProgress class="col-3 offset-9" url="/portal/insurance-companies/new" type="default" :value="100" :striped="false" :animated="true">
      <template slot="header">
        <h5 class="mb-3">افزودن شرکت بیمه جدید</h5>
      </template>
    </CardProgress><!-- -->
    ...
    <div class="col-12">
      ...
      >
        ...
        <template v-slot:cell(actions)="data">
          <router-link :to="{name: 'portal-insurance-companies-edit', params: { id: data.item.id }}"><i class="fas fa-pencil-alt"></i></router-link>
...
export default {
  ...
  data() {
    return {
      fields: [
        { key: 'name', label: 'نام بیمه', sortable: true },
  ...
  methods: {
    ...
    deleteItemOK(){
      let promise = this.$http.delete(this.$api("api/v1/core/insurance-companies/"+this.modalDelete.itemID))
    ...
    myProvider (ctx) {
      ...
      let promise = this.$http.get(this.$api("api/v1/core/insurance-companies"+params))
      console.error(this.$api("api/v1/core/insurance-companies"+params))
