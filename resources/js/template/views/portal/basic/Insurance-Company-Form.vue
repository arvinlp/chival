      ...
      <b-col cols="12">
        <div class="inner-main">
          <div class="inner-header">
            <b-row>
              <b-col cols="11">
                <span v-if="isEdit">ویرایش اطلاعات شرکت « {{form.name}} »</span>
                <span v-else>افزودن اطلاعات</span>
              </b-col>
              <b-col cols="1" class="text-left">
                <router-link to="/portal/insurance-companies" class="btn btn-danger w-100">بازگشت</router-link>
              </b-col>
            </b-row>
          </div>
  ...
  mounted() {
    if (this.$route.params.id) {
      ...
      let promise = this.$http.get(
        this.$api("api/v1/core/insurance-companies/" + this.$route.params.id)
      );
      promise
        .then(response => {
    ...
    onSubmit(evt) {
      ...
      if (this.$route.params.id) {
        ...
        let promise = this.$http.post(
          this.$api("api/v1/core/insurance-companies/new"),
          this.form
        );
        promise
          .then(response => {
            this.$router.push(
              this.$route.query.redirect || "/portal/insurance-companies/"
            );
          })
          .catch(error => {
            this.isAlert = this.dismissSecs;
