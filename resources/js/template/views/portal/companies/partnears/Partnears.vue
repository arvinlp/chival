    ...
    <div class="row">
      ...
      >
        <template slot="header">
          <h5 class="mb-3">افزودن شرکت همکار</h5>
        </template>
      </CardProgress>
      <CardProgress
        class="col-3"
      ...
      <div class="col-12">
        ...
        >
          ...
          <template v-slot:cell(actions)="data">
            <router-link :to="{name: 'portal-company-partnear-edit', params: { id:this.$route.params.id, uid: data.item.id }}" title="ویرایش">
...
export default {
  ...
  data() {
    return {
      fields: [
        { key: "number", label: "شماره قرارداد", sortable: true },
        { key: "company", label: "شرکت اول", sortable: true },
        { key: "partnear", label: "شرکت دوم", sortable: true },
        { key: "expire_date_j", label: "پایان قرارداد", sortable: true },
        {
          key: "status",
  ...
  mounted() {
    if (this.$route.params.id) {
      ...
      this.actionUrl = "partnears"; //"companies/" + this.$route.params.id + "/partnears";
      this.newUrl =
        "/portal/companies/" + this.$route.params.id + "/partnears/new";
      this.api =
        "api/v1/portal/companies/" + this.$route.params.id + "/partnears";
    } else {
      this.actionUrl = "partnears";
      this.newUrl = "/portal/partnears/new";
      this.api = "api/v1/portal/companies/0/partnears";
    }
  },
  methods: {
    ...
    myProvider(ctx) {
      ...
      if (ctx.sortBy == "expire_date_j") {
        sortBy = "expire_date";
      }
      if (ctx.sortBy == "partnear") {
        sortBy = "partnear_id";
      }
      if (ctx.sortBy == "company") {
        sortBy = "company_id";
      }
      if (ctx.sortBy == "number") {
        sortBy = "number";
      }
      let params =
    ...
    statusType(value) {
      switch (parseInt(value)) {
        ...
        case 2:
          return "منتظر تایید";
        case 3:
          return "در انتظار تایید لغو همکاری";
        case 4:
          return "لغو شده";
        default:
          return "---";
