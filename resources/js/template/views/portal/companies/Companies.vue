    ...
    <div class="row">
      ...
      >
        <template slot="header">
          <h5 class="mb-3">افزودن شرکت جدید</h5>
        </template>
      </CardProgress>
      ...
      <div class="col-12">
        ...
        >
          ...
          <template v-slot:cell(actions)="data">
            <router-link :to="`companies/${data.item.id}/users`" title="پرسنل">
              <i class="fas fa-users"></i>
            </router-link>
            <router-link :to="`companies/${data.item.id}/partnears`" title="همکاران">
              <i class="fas fa-file-contract"></i>
            </router-link>
            <router-link :to="`companies/${data.item.id}/routes`" title="مقصدها">
              <i class="fas fa-route"></i>
            </router-link>
            <router-link :to="`companies/${data.item.id}/drivers`" title="رانندها">
              <i class="fas fa-dharmachakra"></i>
              <!-- <i class="fas fa-steering-wheel"></i> -->
            </router-link>
            <router-link :to="`companies/${data.item.id}/vehicles`" title="وسایل نقلیه">
              <i class="fas fa-car"></i>
            </router-link>
            <router-link :to="`companies/${data.item.id}/storage`" title="انبار">
              <i class="fas fa-boxes"></i>
            </router-link>
            <router-link :to="`companies/${data.item.id}/finance`" title="مالی">
              <i class="fas fa-money-bill-alt"></i>
            </router-link>
            <router-link :to="`companies/${data.item.id}/setting`" title="تنظیمات">
              <i class="fas fa-cogs"></i>
            </router-link>
            <router-link :to="{name: 'portal-companies-edit', params: { id: data.item.id }}" title="ویرایش">
...
export default {
  ...
  data() {
    return {
      fields: [
        { key: "name", label: "شرکت", sortable: true },
        { key: "tell", label: "تلفن", sortable: false },
        { key: "mobile", label: "تلفن پشتیبان", sortable: false },
        { key: "location", label: "استان،شهر،شهرستان", sortable: false },
        {
          key: "status",
  ...
  methods: {
    ...
    deleteItemOK() {
      let promise = this.$http.delete(
        this.$api("api/v1/portal/companies/" + this.modalDelete.itemID)
      );
    ...
    myProvider(ctx) {
      ...
      let promise = this.$http.get(
        this.$api("api/v1/portal/companies" + params)
      );
