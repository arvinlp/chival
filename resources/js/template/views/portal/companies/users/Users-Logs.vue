...
export default {
  ...
  data() {
    return {
      fields: [
        ...
        { key: "created_at", label: "ساعت و تاریخ", sortable: false }
      ],
      items: null,
      isBusy: false,
      sortBy: "id",
  ...
  mounted() {
    if (this.$route.params.id) {
      ...
      this.api = "api/v1/portal/companies/users/logs";
      this.apiUser =
        "api/v1/portal/companies/" + this.$route.params.id + "/users/list";
      this.getUsers();
      if (this.$route.params.uid) {
        this.selectedUserId = this.$route.params.uid;
      }
    } else {
  ...
  methods: {
    onSelectUser(value) {
      if (value) this.selectedUserId = value.id;
      else this.selectedUserId = 0;
      this.$root.$emit("bv::refresh::table", "table-one");
    },
    getUsers() {
      let promise = this.$http.get(this.$api(this.apiUser));
      promise
        .then(response => {
    ...
    myProvider(ctx) {
      ...
      if (this.selectedUserId > 0) {
        filtered =
          "&user_type=client&company_id=" +
          this.$route.params.id +
          "&user_id=" +
          this.selectedUserId;
      } else {
        filtered = "&user_type=client&company_id=" + this.$route.params.id;
      }
      let params =
        "?page=" +
        ctx.currentPage +
