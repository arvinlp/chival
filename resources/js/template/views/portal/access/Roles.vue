...
<section class="page">
  <div class="row">
    <CardProgress class="col-3 offset-9" url="/portal/roles/new" type="default" :value="100" :striped="false" :animated="true">
      <template slot="header">
        <h5 class="mb-3">افزودن سطح دسترسی جدید</h5>
      </template>
    </CardProgress><!-- -->
    <div class="col-3">
      <b-form-input
    ...
    <div class="col-5 offset-1">
      ...
      <div class="col-5 mt-2" v-if="selectedCompany > 0">
        <b-button variant="primary" class="w-100" @click="onClickBtn">
          نمایش اطلاعات عمومی
        </b-button>
      </div>
      </div>
    ...
    <div class="col-12">
      <b-table
        ...
        <template v-slot:cell(actions)="data">
          <router-link :to="{name: 'portal-role-edit', params: { id: data.item.id }}"><i class="fas fa-pencil-alt"></i></router-link>
...
export default {
  ...
  data() {
    return {
      fields: [
        ...
        { key: 'status', formatter: 'statusType', label: 'وضعیت استفاده', sortable: true },
  ...
  methods: {
    ...
    deleteItemOK(){
      let promise = this.$http.delete(this.$api("api/v1/portal/roles/"+this.modalDelete.itemID))
    ...
    getCompanies(serach){
      ...
      if(serach)
        params = "?name="+serach
      let promise = this.$http.get(this.$api("api/v1/portal/companies/list"+params))
    ...
    myProvider (ctx) {
      ...
      let params = '?page=' + ctx.currentPage + '&order_by=' + sortBy + '&order=' + sort + compnay_id
      if(ctx.filter){
        params = '?page=' + ctx.currentPage + '&order_by=' + sortBy + '&order=' + sort + compnay_id + '&name=' + ctx.filter
      }

      let promise = this.$http.get(this.$api("api/v1/portal/roles"+params))
